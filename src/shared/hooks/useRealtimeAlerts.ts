        ...
        alertEvents.forEach((eventName) => {
          socket?.on(eventName, (payload: AlertRealtimePayload) => {
            ...
            if (isReportedByCurrentUser) {
              notifyReporter(payload, callbacksRef.current)
...
function notifyReporter(payload: AlertRealtimePayload, callbacks: RealtimeCallbacks) {
  if (payload.event === 'alerts:assigned') {
    ...
    callbacks.showSuccess('Tu reporte fue tomado', 'El equipo tecnico ya esta atendiendo la falla.')
    return
  }
  ...
  if (payload.event === 'alerts:resolved') {
    ...
    callbacks.showSuccess('Tu reporte fue resuelto', payload.alert.title)
    return
  }
  ...
  if (payload.event === 'alerts:note_added') {
    ...
    callbacks.showSuccess('Tu reporte tiene una nota nueva', payload.alert.title)
    return
  }
  ...
  if (payload.event === 'alerts:dismissed') {
    ...
    callbacks.showSuccess('La notificacion de tu reporte fue archivada', payload.alert.title)
  }
}
