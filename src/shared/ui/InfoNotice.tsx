...
export function InfoNotice({ autoCloseMs = 5200, message, onClose, subText }: InfoNoticeProps) {
  useEffect(() => {
    if (!onClose) return
  ...
  return (
    ...
    <div className="notice-toast relative flex min-h-20 w-[calc(100vw-2rem)] items-center justify-around gap-4 overflow-hidden rounded-lg bg-white px-4 py-3 shadow-lg shadow-slate-950/10 sm:max-w-[330px]">
      <svg
        aria-hidden="true"
        className="absolute -left-8 top-8 h-20 w-20 rotate-90 fill-blue-500/25"
      ...
      <div className="relative ml-2 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-blue-500/25">
        <svg
      ...
      <div className="relative min-w-0 flex-1">
        <p className="text-base font-bold text-blue-700">{message}</p>
      ...
      {onClose && (
        <button
          ...
          onClick={onClose}
        >
          <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={0} viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg">
            <path
              clipRule="evenodd"
